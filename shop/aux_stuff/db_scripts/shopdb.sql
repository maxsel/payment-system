...
CREATE TABLE IF NOT EXISTS `shopdb`.`item` (
  ...
  `img_format` VARCHAR(45) NULL,
  `img_filename` VARCHAR(45) NULL,
  PRIMARY KEY (`item_id`),
...
CREATE TABLE IF NOT EXISTS `shopdb`.`order_has_item` (
  `order_has_item_id` INT NOT NULL AUTO_INCREMENT,
  `order_id` INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS `shopdb`.`item_in_cart` (
  ...
  CONSTRAINT `fk_item-in-cart_item1`
    ...
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_item_in_cart_user1`
    FOREIGN KEY (`user_id`)
    REFERENCES `shopdb`.`user` (`user_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
SHOW WARNINGS;
CREATE INDEX `fk_item_in_cart_user1_idx` ON `shopdb`.`item_in_cart` (`user_id` ASC);
